...
void bench_sdpa_primitives(engine::kind ekind, memory::data_type dt,
        const sdpa_dims_t &p, bool has_concat = false) {
    ...
    concat_src_mds.push_back(output_md);
    concat_src_mds.push_back(output_md);
    concat_src_mems.push_back(m_output);
    concat_src_mems.push_back(m_output);
    auto concat_pd = concat::primitive_desc(eng, axis, concat_src_mds);
    auto concat_prim = concat(concat_pd);
    auto concat_dst_mem = memory(concat_pd.dst_desc(), eng);
    std::unordered_map<int, memory> concat_args;
    concat_args.insert({DNNL_ARG_MULTIPLE_SRC, concat_src_mems[0]});
    concat_args.insert({DNNL_ARG_MULTIPLE_SRC + 1, concat_src_mems[1]});
    ...
    const auto loop = [&]() {
        ...
        // ======= Add extra concat ========= //
        if (has_concat) {
            concat_prim.execute(strm, concat_args);
        }
    };
...
void bench_sdpa(engine::kind ekind, logical_tensor::data_type dt,
    ...
    std::vector<memory> concat_src_mems;
    const memory::dims q_sz = {p.mb, p.head_num, p.query_num, p.head_size};
    auto output_md = memory::desc(q_sz, memory::data_type::f16, tag::abcd);
    auto m_output = memory(output_md, eng);
    write_to_dnnl_memory(value_data.data(), m_output);
    concat_src_mds.push_back(output_md);
    concat_src_mds.push_back(output_md);
    concat_src_mems.push_back(m_output);
    concat_src_mems.push_back(m_output);
    ...
    cp.execute(
            strm, {ts_query, ts_key, ts_value}, {ts_output});
    ...
    auto start = std::chrono::steady_clock::now();
    for (int i = 0; i <= runs; i++) {
        cp.execute(strm, {ts_query, ts_key, ts_value},
                {ts_output});
        // ======= Add extra concat ========= //
        if (has_concat) {
            concat_prim.execute(strm, concat_args);
...
void bad_args() {
    std::cerr << "Usage: graph-sdpa-cpp \n"
                 "       graph-sdpa-cpp <query_num> <seq_len>\n\n";
    ...
    try {
        if (api == api_kind::primitive) {
            bench_sdpa_primitives(
                    ekind, static_cast<memory::data_type>(dt), p, has_concat);
        } else {
            // api == api_kind::graph
            bench_sdpa(ekind, static_cast<logical_tensor::data_type>(dt), p, has_concat);
            get_mem_pool().clear();
        }
    } catch (dnnl::error &e) {
...
void sdpa_perf(engine::kind ekind, int argc, char **argv) {
    ...
    sdpa_dims_t params = {1, 256, 8, 32, 256};

    // ([1, 8, 256, 32]  [1, 8, 256, 32] [1, 8, 256, 32])
    std::cout << "Shape: ([1, 8, 256, 32]  [1, 8, 256, 32] [1, 8, 256, 32])" << std::endl;
    params = {1, 256, 8, 32, 256};
    std::cout << "### micro_sdpa" << std::endl;
    bench(api_kind::graph, ekind, dnnl_f16, params, false);
    std::cout << "### micro_sdpa + concat" << std::endl;
    bench(api_kind::graph, ekind, dnnl_f16, params, true);
    // std::cout << "matmul+softmax+matmul" << std::endl;
    // bench(api_kind::primitive, ekind, dnnl_f16, params, false);
    // std::cout << "matmul+softmax+matmul + concat" << std::endl;
    // bench(api_kind::primitive, ekind, dnnl_f16, params, true);
    std::cout << std::endl;

    //  ([1, 8, 100, 32] [1, 8, 100, 32] [1, 8, 100, 32]) 
    std::cout << "Shape: ([1, 8, 100, 32] [1, 8, 100, 32] [1, 8, 100, 32])" << std::endl;
    params = {1, 100, 8, 32, 100};
    std::cout << "### micro_sdpa" << std::endl;
    bench(api_kind::graph, ekind, dnnl_f16, params, false);
    std::cout << "### micro_sdpa + concat" << std::endl;
    bench(api_kind::graph, ekind, dnnl_f16, params, true);
    ...
    // ([1, 8, 100, 32]  [1, 8, 256, 32] [[1, 8, 256, 32]])
    std::cout << "Shape: ([1, 8, 100, 32]  [1, 8, 256, 32] [[1, 8, 256, 32]])" << std::endl;
    params = {1, 256, 8, 32, 100};
    std::cout << "### micro_sdpa" << std::endl;
    bench(api_kind::graph, ekind, dnnl_f16, params, false);
    std::cout << "### micro_sdpa + concat" << std::endl;
    bench(api_kind::graph, ekind, dnnl_f16, params, true);
...
int main(int argc, char **argv) {
    return handle_example_errors(
            sdpa_perf, engine::kind::gpu, argc, argv);
}
