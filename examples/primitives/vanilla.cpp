...
/// @example vanilla.cpp
/// > Annotated version: @ref vanilla_example_cpp
///
/// @page vanilla_example_cpp_short
///
/// This C++ API example demonstrates how to create and execute a
/// [Vanilla RNN](@ref dev_guide_rnn) primitive in forward inference propagation
/// mode.
///
/// Key optimizations included in this example:
/// - Creation of optimized memory format from the primitive descriptor.
///
/// @page vanilla_example_cpp Vanilla RNN Primitive Example
/// @copydetails vanilla_example_cpp_short
///
/// @include vanilla.cpp
...
void vanilla_example(dnnl::engine::kind engine_kind) {
    ...
    auto vanilla_weights_layer_md
            = memory::desc(weights_dims, dt::f32, tag::any);
    auto vanilla_weights_iter_md
            = memory::desc(weights_dims, dt::f32, tag::any);
    ...
    auto vanilla_pd = vanilla_rnn_forward::primitive_desc(engine,
            prop_kind::forward_inference, dnnl::algorithm::eltwise_tanh,
            rnn_direction::unidirectional_left2right, src_layer_md, src_iter_md,
            vanilla_weights_layer_md, vanilla_weights_iter_md, bias_md,
    ...
    auto vanilla_weights_layer_mem = user_weights_layer_mem;
    auto vanilla_weights_iter_mem = user_weights_iter_mem;
    ...
    if (vanilla_pd.weights_desc() != user_weights_layer_mem.get_desc()) {
        vanilla_weights_layer_mem = memory(vanilla_pd.weights_desc(), engine);
        reorder(user_weights_layer_mem, vanilla_weights_layer_mem)
                .execute(engine_stream, user_weights_layer_mem,
                        vanilla_weights_layer_mem);
    }

    if (vanilla_pd.weights_iter_desc() != user_weights_iter_mem.get_desc()) {
        vanilla_weights_iter_mem
                = memory(vanilla_pd.weights_iter_desc(), engine);
        reorder(user_weights_iter_mem, vanilla_weights_iter_mem)
                .execute(engine_stream, user_weights_iter_mem,
                        vanilla_weights_iter_mem);
    }

    // Create the memory objects from the primitive descriptor.
    auto src_iter_mem = memory(vanilla_pd.src_iter_desc(), engine);
    auto weights_iter_mem = memory(vanilla_pd.weights_iter_desc(), engine);
    auto dst_iter_mem = memory(vanilla_pd.dst_iter_desc(), engine);

    // Create the primitive.
    auto vanilla_prim = vanilla_rnn_forward(vanilla_pd);

    // Primitive arguments
    std::unordered_map<int, memory> vanilla_args;
    vanilla_args.insert({DNNL_ARG_SRC_LAYER, src_layer_mem});
    vanilla_args.insert({DNNL_ARG_WEIGHTS_LAYER, vanilla_weights_layer_mem});
    vanilla_args.insert({DNNL_ARG_WEIGHTS_ITER, vanilla_weights_iter_mem});
    vanilla_args.insert({DNNL_ARG_BIAS, bias_mem});
    vanilla_args.insert({DNNL_ARG_DST_LAYER, dst_layer_mem});
    vanilla_args.insert({DNNL_ARG_SRC_ITER, src_iter_mem});
    vanilla_args.insert({DNNL_ARG_DST_ITER, dst_iter_mem});

    // Primitive execution: vanilla.
    vanilla_prim.execute(engine_stream, vanilla_args);
...
int main(int argc, char **argv) {
    return handle_example_errors(
            vanilla_example, parse_engine_kind(argc, argv));
}
