...
///
/// @page lbr_gru_example_cpp Linear-Before-Reset RNN Primitive Example
...
void lbr_gru_example(dnnl::engine::kind engine_kind) {
    ...
    // dimensions.
    memory::dims src_dims = {T, N, C1};
    memory::dims weights_layer_dims = {L, D, C1, G, C2};
    memory::dims weights_iter_dims = {L, D, C2, G, C2};
    memory::dims bias_dims = {L, D, G + E, C2};
    memory::dims dst_layer_dims = {T, N, C2};
    memory::dims dst_iter_dims = {L, D, N, C2};
    ...
    auto lbr_gru_weights_layer_md
            = memory::desc(weights_layer_dims, dt::f32, tag::any);
    auto lbr_gru_weights_iter_md
    ...
    auto lbr_gru_pd = lbr_gru_forward::primitive_desc(engine,
            ...
            lbr_gru_weights_layer_md, lbr_gru_weights_iter_md, bias_md,
    ...
    auto lbr_gru_weights_layer_mem = user_weights_layer_mem;
    auto lbr_gru_weights_iter_mem = user_weights_iter_mem;
    ...
    if (lbr_gru_pd.weights_desc() != user_weights_layer_mem.get_desc()) {
        lbr_gru_weights_layer_mem = memory(lbr_gru_pd.weights_desc(), engine);
        reorder(user_weights_layer_mem, lbr_gru_weights_layer_mem)
                .execute(engine_stream, user_weights_layer_mem,
                        lbr_gru_weights_layer_mem);
    }

    if (lbr_gru_pd.weights_iter_desc() != user_weights_iter_mem.get_desc()) {
        lbr_gru_weights_iter_mem
                = memory(lbr_gru_pd.weights_iter_desc(), engine);
        reorder(user_weights_iter_mem, lbr_gru_weights_iter_mem)
    ...
    auto weights_iter_mem = memory(lbr_gru_pd.weights_iter_desc(), engine);
    ...
    lbr_gru_args.insert({DNNL_ARG_WEIGHTS_LAYER, lbr_gru_weights_layer_mem});
    lbr_gru_args.insert({DNNL_ARG_WEIGHTS_ITER, lbr_gru_weights_iter_mem});
